 ...
 * Task 1
 */
const simpleArraySum = xs => xs.reduce((acc, i) => acc + i, 0);
...
const generateRandomItemList = n => {
  ...
  let randomSize = ["small", "medium", "large"];

  let array = [];

  Array.from({ length: n }).forEach((x, i) => {
    array.push({
      "name": "product_" + getRandomIntInclusive(0, 3),
...
const findMostExpensiveItem = items => {
  return items.sort((a, b) => { return b.cost - a.cost;})[0];
};
...
const findCheapestItem = items => {
  return items.sort((a, b) => { return a.cost - b.cost;})[0];
};
/**
 * Task 6
 * 
